...
from src.rae_batch import BilingualPhraseRAEBiLinear, brae_predict
from src.data_utils.phrase_utils import read_phrase_pair_vocab, read_phrase_list, filter_vocab, clean_text, WORD_INDEX
...
def pre_model(src_dict, tar_dict, config, verbose):
    if rand_word_init:
        ...
        en_embedding_name = "data/embedding/en.token.min%d.dim%d.bin" % (config.min_count, config.dim)
        zh_embedding_name = "data/embedding/zh.token.min%d.dim%d.bin" % (config.min_count, config.dim)
    ...
    return BilingualPhraseRAEBiLinear(src_embedding, tar_embedding, config=config, verbose=verbose)
...
def main():
    ...
    if train_test == "train":
        ...
        if start_iter == 0:
            print "Load Dict ..."
            en_embedding_name = "data/embedding/en.token.min%d.dim%d.bin" % (brae_config.min_count, brae_config.dim)
            zh_embedding_name = "data/embedding/zh.token.min%d.dim%d.bin" % (brae_config.min_count, brae_config.dim)
